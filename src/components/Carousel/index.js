export default class Carousel {
  ...
  slider(event) {
    ...
    if (event.target.closest(".carousel__arrow_left")) {
      // [event.target] - это исходный элемент, на котором произошло событие(элемент на который мы кликнули); [closest] - проверяет кликнули ли мы на нужный нам элементе .carousel__arrow_left или элемент вложенный в него
      this.elem.querySelector(
        ".carousel__inner"
      ).style.transform = `translateX(${(this.value += counter)}px)`; // задаем перемещение слайдов на их ширину влево от this.value
    ...
    if (event.target.closest(".carousel__arrow_right")) {
      //*свойстов [event.target] - содержит элемент, на котором сработало событие; [closest] - ищет ближайший родительский элемент подходящий указаному классу*/
      this.elem.querySelector(
        ".carousel__inner"
      ...
      if (this.curentSlide === this.slideLenght.length) {
        // если номер слайда = количеству слайдов
  ...
  carousel(slides) {
    ...
    carousel.classList.add(
      "carousel"
    ); /* создаем class = 'carousel' для только что созданого <div>*/
    ...
    divCarouselInner.classList.add(
      "carousel__inner"
    ); /* добавлем к <div> class = "carousel__inner"*/

    for (const value of slides) {
      /**при помощи цикла for of создаем html блоки для всех имеющихся слайдов */

      let divCarouselSlide = document.createElement("div"); /* создаем <div> */
      divCarouselSlide.classList.add(
        "carousel__slide"
      ); /* добавляем к <div> class = "carousel__slide"*/
      divCarouselSlide.setAttribute(
        "data-id",
        `${value.id}`
      ); /* изменяем значение атрибута <div> */

      divCarouselSlide.insertAdjacentHTML(
        "afterbegin",
        `                                                    /* добавляем Html посел опорного элемента*/
      <img src="/assets/images/carousel/${
        value.image
      }" class="carousel__img" alt="slide">                   /* Изображаения берем из свойства image. А картинки img с CSS классом carousel__img  */
